...
H~A~: **pH effects abalone shell growth.**
...
abalone_clean <- abalone  |> 
  # cleaning up column names to remove spaces
  clean_names()  |> 
...
# base layer: ggplot
ggplot(data = abalone_clean, # selecting cleaned data set
       aes(x = mean_ph, # setting x and y axis to be mean ph and change in area respectively
           y = change_in_area)) +
  # first layer: point graph
  geom_point(size = 4,
             stroke = 1,
             fill = "firebrick4",
             shape = 21)
...
**the missing values comes from one pair of tanks where the pH sensor failed.**
...
**At a mean ph of 8 the midel predicts a change in abalone shell surface area (mm^2/day) to be 5.36 [95% CI: 5.08, 5.64]**
